...
export default () => {
  ...
  const [frequencyAmount, setFrequencyAmount] = useState(5);
  ...
  useEffect(() => {
    ...
    if(frequencyAmount !== "" && timeSpentAmount !== ""){
      setTimePerEvaluation(totalSecondsUsedPerYear * frequencyMapping[evaluation]);
    }
  }, [frequencyAmount, frequencyUnit, timeSpentAmount, timeSpentUnit, evaluation]);
